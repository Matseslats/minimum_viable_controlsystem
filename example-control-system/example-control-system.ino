...
#include <math.h>
// Reason for weird import: https://forum.arduino.cc/t/settime-was-not-declared-in-this-scope/399739/7
#include "TimeLib.h"
#include "TinyGPS.h"
#include <EEPROM.h>
...
static void say(byte level, String msg) {
...
void loop() {
  ...
  int wp_hdg = 0;
  float wp_dist = 0.0;
  byte wp_num = EEPROM.read(WP_ADDR); // Max 255 waypoints, should be fine
  ...
  while (1) {
    ...
    if (millis() - last_gps_read > (GPS_READ_INTERVAL * 1000) || millis() < last_gps_read) { // read the GPS at the specified interval or whenever the millis count wraps around
      ...
      if (wp_dist < WP_THRESHOLD) {
        if (wp_num != NUM_OF_WAYPOINTS) { // If this isnt the last waypoint
          wp_num++;
          say(DEBUG_CRITICAL, "This would write to eeprom");
          // EEPROM.update(WP_ADDR, wp_num); // Store in case of reboot
        }
